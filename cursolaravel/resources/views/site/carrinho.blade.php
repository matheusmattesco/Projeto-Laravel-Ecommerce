    ...
    <table class="striped">
        ...
        <tbody>
          ...
          <tr>
            ...
            {{-- BTN ATUALIZAR --}}
            <form action="{{ route('site.atualizacarrinho') }}" method="POST" enctype="multipart/form">
                @csrf
                <input type="hidden" name="id" value="{{ $item->id }}">
            <td><input style="width: 40px" type="number" min="1" name="quantity" value="{{$item->quantity}}"></td>
            <td> 
            <button class="btn-floating waves-effect waves-light orange"><i class="material-icons">refresh</i></button>
            </form>
                
            {{-- BTN REMOVER --}}
                <form action="{{ route('site.removecarrinho')}}" method="POST" enctype="multipart/form">
                @csrf
                <input type="hidden" name="id" value="{{ $item->id }}">
      ...
      <div class="card orange">
        <div class="card-content white-text">
          <span class="card-title right" style="font-weight:600">Total: R$ {{ number_format(\Cart::getTotal(), 2, ',', '.')}}</span>
